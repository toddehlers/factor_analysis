...
% Factor Analysis script by J. Starke, last modified Mai 2017
% University of Tuebingen, Department of Geosciences
% Earth Systems Dynamics Research Group
%xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

% This script was originally developed by K. Hartmann and J. Starke in 2013, and
% published in Starke et al., 2017.

% If you publish results from this script or a modified version of it please cite
% Starke, J., Ehlers, T. A., & Schaller, M. (2017). Tectonic and
% Climatic Controls on the Spatial Distribution of Denudation Rates in Northern
% Chile (18°S to 23°S) Determined From Cosmogenic Nuclides: 10 Be Denudation
% Rates in Northern Chile. Journal of Geophysical Research:
% Earth Surface. https://doi.org/10.1002/2016JF004153
% Corresponding author: Todd Ehlers todd.ehlers@uni-tuebingen.de

% for further information see also Hartmann & Wuennemann 2009: Hydrological
% changes and Holocene climate variations in NW China, inferred from lake
% sediments of Juyanze palaeolake by factor analyses
...
lat = data(:,1); %latitude
long = data (:,2); %longitude
X=data(:,3:end);

%% Select type of transformation

M=input('Please use the transformation type 2 because 1 and 3 are not suitable for the test dataset!Thanks! \n 1 = Centered log-ratio-tansformation with geometric mean (warning do not use for this data set!) \n 2 = Z-transformation \n 3 = W-transformation to (0,1) after Manson & Imbrie (1964) \n');
...
if abs(Corr_inv_notdiag_mean) < 2 % here you can adjust treshold
    disp(' ')
    disp('the mean of the non diagonal elements is <2, the input data matrix is suitable for the factor analysis')
    ...
    disp(' ')
    disp('the mean of the non diagonal elements is >2, the input data matrix is not suitable for the factor analysis')
    disp('              meannondiag-meandiag     mean       max       min')
    fprintf('                  %6.2f            %6.2f      %6.2f     %6.2f\n', Corr_inv_notdiag_mean-Corr_inv_diag_mean,mean(Corr_inv_notdiag_mean),max(max(Corr_inv-diag(diag(Corr_inv)))),min(min(Corr_inv-diag(diag(Corr_inv)))))
end
...
if m/length(Corr)^2>0.25
	disp(' ')
    fprintf('%u %% of the non diagonal values of the anti image covarianz are grater than 0 \n-> the input data matrix is not suitable for the factor analysis\n', m/length(Corr)^2*100)
	disp(' ')
elseif m/length(Corr)^2<0.25
    disp(' ')
    fprintf('%u %% of the non diagonal values of the anti image covarianz are greater than 0 \n-> the input data matrix is suitable for the factor analysis\n', m/length(Corr)^2*100)
	disp(' ')
end
...
WM_ZG = acos(Corr)/pi*180;%Winkelmatrix in GRAD
...
title('How to select number of factors?!','FontSize', 20);
